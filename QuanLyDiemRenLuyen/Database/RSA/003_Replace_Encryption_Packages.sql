...
CREATE OR REPLACE PACKAGE BODY PKG_STUDENT_ENCRYPTION AS
    ...
    BEGIN
        IF p_phone IS NULL OR LENGTH(TRIM(p_phone)) = 0 THEN
            RETURN NULL;
        END IF;
        RETURN TO_CLOB(PKG_RSA.ENCRYPT_WITH_SYSTEM_KEY(p_phone));
    END ENCRYPT_PHONE;
    ...
    BEGIN
        ...
        RETURN PKG_RSA.DECRYPT_WITH_SYSTEM_KEY(v_encrypted_str);
    EXCEPTION
        WHEN OTHERS THEN
    ...
    BEGIN
        IF p_address IS NULL OR LENGTH(TRIM(p_address)) = 0 THEN
            RETURN NULL;
        END IF;
        RETURN TO_CLOB(PKG_RSA.ENCRYPT_WITH_SYSTEM_KEY(p_address));
    END ENCRYPT_ADDRESS;
    ...
    BEGIN
        IF p_id_card IS NULL OR LENGTH(TRIM(p_id_card)) = 0 THEN
            RETURN NULL;
        END IF;
        RETURN TO_CLOB(PKG_RSA.ENCRYPT_WITH_SYSTEM_KEY(p_id_card));
    END ENCRYPT_ID_CARD;
...
CREATE OR REPLACE PACKAGE BODY PKG_SCORE_SIGNATURE AS
    ...
    BEGIN
        ...
        RETURN TO_CLOB(PKG_RSA.SIGN_WITH_SYSTEM_KEY(v_data));
    END SIGN_SCORE;
    ...
    BEGIN
        ...
        RETURN PKG_RSA.VERIFY_WITH_SYSTEM_KEY(v_data, v_sig_str);
...
CREATE OR REPLACE PACKAGE BODY PKG_FEEDBACK_ENCRYPTION AS
    ...
    BEGIN
        IF p_content IS NULL OR LENGTH(TRIM(p_content)) = 0 THEN
            RETURN NULL;
        END IF;
        RETURN TO_CLOB(PKG_RSA.ENCRYPT_WITH_SYSTEM_KEY(p_content));
    END ENCRYPT_CONTENT;
